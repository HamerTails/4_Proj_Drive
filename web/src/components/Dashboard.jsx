...
const FileIcon = ({ category, size = 32 }) => (
  <img src={FILE_ICONS[category]} alt={category} width={size} height={size} style={{ display: 'block', flexShrink: 0 }} />
);
...
export default function Dashboard({ sharedOnly = false }) {
  ...
  const [draggedNode, setDraggedNode]     = useState(null);  // node en cours de drag
  const [dropTarget, setDropTarget]       = useState(null);  // id du dossier survolé
  const [isDraggingActive, setIsDraggingActive] = useState(false); // curseur grab seulement pendant le drag
  ...
  // drop sur "Accueil" = déplacer à la racine (parent_id = null)
  ...
  const onDrop = (e) => {
    e.preventDefault();
    setDragging(false);
    // si on drag un node (pas un fichier externe) on ignore
    if (draggedNode) return;
  ...
  const displayNodes = search.trim()
    ? baseNodes.filter(n => n.name.toLowerCase().includes(search.trim().toLowerCase()))
    : baseNodes;

  // style d'une row/card selon son état drag
  const rowStyle = (node) => ({
  ...
  return (
    <div
      ...
      onDragOver={(e) => { e.preventDefault(); if (!draggedNode) setDragging(true); }}
    ...
    >
      ...
      <div className="topbar">
        ...
        {!sharedOnly && (
          <div className="search-bar">
            ...
            <input type="text" placeholder="Rechercher des fichiers..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        )}
        ...
        {!sharedOnly && (
          <div className="topbar-actions">
            <div style={{ display: 'flex', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
              <button className="btn-icon" title="Vue liste" style={{ borderRadius: 0, background: viewMode === 'list' ? 'var(--bg-tertiary)' : 'transparent', color: viewMode === 'list' ? 'var(--text-primary)' : 'var(--text-tertiary)' }} onClick={() => toggleView('list')}><IconList /></button>
              <button className="btn-icon" title="Vue grille" style={{ borderRadius: 0, background: viewMode === 'grid' ? 'var(--bg-tertiary)' : 'transparent', color: viewMode === 'grid' ? 'var(--text-primary)' : 'var(--text-tertiary)' }} onClick={() => toggleView('grid')}><IconGrid /></button>
            </div>
            ...
            <input ref={fileInputRef} type="file" multiple className="upload-input" onChange={(e) => handleUpload([...e.target.files])} />
          </div>
        )}
      </div>
      ...
      <div className="page-content">
        {!sharedOnly && (
          <div className="breadcrumb">
            <button
              ...
              onDragOver={(e) => { if (draggedNode) { e.preventDefault(); setDropTarget('root'); }}}
              onDragLeave={() => setDropTarget(null)}
              onDrop={(e) => { setDropTarget(null); handleDropOnRoot(e); }}
              style={{
            ...
            {breadcrumb.map((item, i) => (
              <span key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <button className={`breadcrumb-item ${i === breadcrumb.length - 1 ? 'current' : ''}`} onClick={() => i < breadcrumb.length - 1 && setCurrentFolder(item.id)}>{item.name}</button>
              </span>
            ))}
          </div>
        )}
        ...
        {search.trim() && (
          <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span className="badge badge-blue">{displayNodes.length} résultat{displayNodes.length !== 1 ? 's' : ''} pour « {search} »</span>
            <button className="btn btn-ghost" style={{ height: 26, fontSize: 12 }} onClick={() => setSearch('')}>Effacer</button>
          </div>
        )}

        {loading ? (
          <div className="empty-state"><div style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>Chargement…</div></div>
        ) : displayNodes.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon">{search.trim() ? '🔍' : sharedOnly ? '📥' : '📂'}</div>
            <div className="empty-state-title">{search.trim() ? 'Aucun fichier trouvé' : sharedOnly ? 'Aucun partage' : 'Dossier vide'}</div>
            <div className="empty-state-desc">{search.trim() ? `Aucun fichier ne correspond à « ${search} »` : sharedOnly ? 'Personne ne vous a partagé de fichier.' : 'Glissez des fichiers ici ou cliquez sur "Uploader".'}</div>
          </div>
        ) : viewMode === 'list' ? (
            ...
            {displayNodes.map((node) => {
              ...
              return (
                <div
                  ...
                  onDragOver={isFolder ? (e) => handleDragOverFolder(e, node.id) : (e) => e.preventDefault()}
            ...
            {displayNodes.map((node) => {
              ...
              return (
                <div
                  ...
                  onMouseEnter={e => { if (!isDrop) { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.boxShadow = 'var(--shadow-md)'; }}}
                  onMouseLeave={e => { if (!isDrop) { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none'; }}}
                >
                  {!sharedOnly && (
                    <div style={{ position: 'absolute', top: 6, right: 6, display: 'flex', gap: 2, opacity: 0 }} className="grid-actions" onClick={(e) => e.stopPropagation()}>
                      <button className="btn-icon" style={{ width: 26, height: 26 }} title="Renommer" onClick={() => { setRenameNode(node); setRenameName(node.name); }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      </button>
                      <button className="btn-icon" style={{ width: 26, height: 26, color: 'var(--danger)' }} title="Supprimer" onClick={() => handleDelete(node)}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg>
                      </button>
                    </div>
                  )}
                  <FileIcon category={cat} size={48} />
                  <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', textAlign: 'center', wordBreak: 'break-word', lineHeight: 1.3, maxWidth: '100%' }}>{node.name}</span>
                  <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{node.type === 'file' ? formatSize(node.size) : 'dossier'}</span>
                </div>
              );
            })}
          </div>
      ...
      {newFolderOpen && (
        <div className="modal-overlay" onClick={() => setNewFolderOpen(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Nouveau dossier</span><button className="btn-icon" onClick={() => setNewFolderOpen(false)}>✕</button></div>
            <form onSubmit={handleCreateFolder}>
              <div className="modal-body"><div className="form-group"><label className="form-label">Nom du dossier</label><input className="form-input" type="text" value={newFolderName} onChange={e => setNewFolderName(e.target.value)} autoFocus required placeholder="Mon dossier" /></div></div>
              <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={() => setNewFolderOpen(false)}>Annuler</button><button type="submit" className="btn btn-primary">Créer</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {renameNode && (
        <div className="modal-overlay" onClick={() => setRenameNode(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Renommer</span><button className="btn-icon" onClick={() => setRenameNode(null)}>✕</button></div>
            <form onSubmit={handleRename}>
              <div className="modal-body"><div className="form-group"><label className="form-label">Nouveau nom</label><input className="form-input" type="text" value={renameName} onChange={e => setRenameName(e.target.value)} autoFocus required /></div></div>
              <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={() => setRenameNode(null)}>Annuler</button><button type="submit" className="btn btn-primary">Renommer</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {previewFile && (
        <div className="modal-overlay" onClick={() => { setPreviewFile(null); setPreviewText(''); }}>
          <div className="modal modal-preview" style={{ maxWidth: '96vw', width: '96vw' }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">{previewFile.name}</span>
              <div style={{ display: 'flex', gap: 6 }}>
                <a href={fileService.downloadFile(previewFile.id)} download><button className="btn btn-secondary" style={{ height: 30, fontSize: 12 }}>Télécharger</button></a>
                <button className="btn-icon" onClick={() => { setPreviewFile(null); setPreviewText(''); }}>✕</button>
              </div>
            </div>
            <div className="modal-body">
              {isImage(previewFile.mime_type) && <img src={`${API_URL}/api/files/${previewFile.id}/preview?token=${token}`} alt={previewFile.name} style={{ maxWidth: '100%', maxHeight: '85vh', borderRadius: 8 }} />}
              {isPdf(previewFile.mime_type) && <iframe src={`${API_URL}/api/files/${previewFile.id}/preview?token=${token}`} title={previewFile.name} style={{ width: '92vw', height: '85vh', border: 'none', borderRadius: 8 }} />}
              {isText(previewFile.mime_type, previewFile.name) && <pre className="preview-text">{previewText || 'Chargement…'}</pre>}
              {isAudio(previewFile.mime_type) && <audio controls autoPlay style={{ width: '100%', marginTop: 20 }} src={`${API_URL}/api/files/${previewFile.id}/stream?token=${token}`} />}
              {isVideo(previewFile.mime_type) && <video controls autoPlay style={{ maxWidth: '100%', maxHeight: '82vh', borderRadius: 8 }} src={`${API_URL}/api/files/${previewFile.id}/stream?token=${token}`} />}
            </div>
          </div>
        </div>
      )}
      ...
      {shareNode && (
        <div className="modal-overlay" onClick={() => setShareNode(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Partager — {shareNode.name}</span><button className="btn-icon" onClick={() => setShareNode(null)}>✕</button></div>
            <div className="modal-body">
                ...
                <div>
                  ...
                  : shareLink ? (
                    <div>
                      ...
                      <a href={shareLink} target="_blank" rel="noopener noreferrer" className="btn btn-secondary" style={{ fontSize: 12, height: 30 }}>Ouvrir ↗</a>
                    </div>
                  ) : <button className="btn btn-primary" onClick={generateLink}>Générer le lien</button>}
                </div>
              ) : (
                <form onSubmit={handleInternalShare}>
                  <div className="form-group"><label className="form-label">Email de l'utilisateur</label><input className="form-input" type="email" value={shareEmail} onChange={e => setShareEmail(e.target.value)} placeholder="utilisateur@example.com" required autoFocus /></div>
                  {shareStatus && <div style={{ marginBottom: 12, fontSize: 13, color: shareStatus.includes('✓') ? 'var(--success)' : 'var(--danger)' }}>{shareStatus}</div>}
