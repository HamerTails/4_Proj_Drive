...
router.post("/register", async (req, res) => {
  try {
    ...
    if (existingUser.rows.length > 0) return res.status(409).json({ error: "Cet email est déjà utilisé" });
...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = result.rows[0];
    if (!user.password_hash) return res.status(401).json({ error: "Utilisez la connexion Google" });

    const valid = await bcrypt.compare(password, user.password_hash);
    if (!valid) return res.status(401).json({ error: "Identifiants incorrects" });

    const token = jwt.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: "7d" });
    res.json({ message: "Connexion réussie", user: { id: user.id, email: user.email }, token });
  } catch (error) {
